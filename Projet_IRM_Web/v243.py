...
# --- 1. CONFIGURATION ---
st.set_page_config(layout="wide", page_title="Magnetovault V243 - Anatomy Restore")
...
# --- 3. CONSTANTES PHYSIQUES (Celles de la V232 EXACTES) ---
# Ces valeurs donnaient le bon contraste visuel
...
# --- 9. SLIDERS (CLÉS INTELLIGENTES) ---
current_reset_id = st.session_state.reset_count
# Pour TR/TE/TI : On inclut le nom de la séquence dans la clé
# Cela force la mise à jour des defaults quand on change de T1 à T2
...
# Pour la géométrie, clé simple basée sur le Reset (persiste entre T1/T2)
...
# --- 13. AFFICHAGE FINAL ---
st.title("Simulateur MagnétoVault V243")
...
with t2:
    ...
    with col_k1:
        ...
        # Clef avec reset_id pour remise à zéro
...
with t3:
    ...
    # V233 fix (couleur grise pour GM) conservé
...
# TAB 5 (ANATOMIE - RETOUR V232 EXACT)
with t5:
    ...
    if HAS_NILEARN and processor.ready:
        ...
        with c2:
            ...
            if img_raw is not None:
                # --- RETOUR LOGIQUE V232 (Sans auto-norm) ---
...
with t6:
    ...
    # GM en dimgray (correction conservée)
