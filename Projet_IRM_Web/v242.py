...
# --- 1. CONFIGURATION ---
st.set_page_config(layout="wide", page_title="Magnetovault V242 - Gold Master")
...
# --- 3. CONSTANTES PHYSIQUES (Anatomie V236) ---
...
# On utilise une clé simple liée au reset pour le sélecteur
seq_key = f"seq_select_{st.session_state.reset_count}"

# Si la variable interne ne correspond pas (ex: après un reset manuel dans le code), on force l'index
try:
    idx_default = options_seq.index(st.session_state.seq)
except:
    idx_default = 0
...
# C'est ici que la magie opère pour retrouver les réglages
...
# --- 9. SLIDERS INTELLIGENTS ---
st.sidebar.header("1. Chrono (ms)")
is_ir = "FLAIR" in seq_choix or "STIR" in seq_choix

# ASTUCE : La clé contient le nom de la séquence. 
# Si on change de séquence, la clé change -> Le slider est recréé avec la nouvelle valeur par défaut.
current_reset_id = st.session_state.reset_count
seq_id = seq_choix.replace(" ", "_") # Nettoyage nom pour clé
...
# Pour la géométrie, on ne veut pas que ça change si on change de séquence (T1 -> T2),
# Sauf si on fait un "Reset Complet". Donc la clé ne dépend QUE du reset_count.
...
turbo = st.sidebar.slider("Turbo (Facteur de Train)", 1, 16, 1, key=f"turbo_{current_reset_id}")
es = st.sidebar.slider("TE Mini (Espacement)", 5.0, 50.0, 15.0, step=1.0, key=f"es_{current_reset_id}")
...
# --- 13. AFFICHAGE FINAL ---
st.title("Simulateur MagnétoVault V242")
